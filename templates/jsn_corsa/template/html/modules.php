...
function modChrome_jsnmodule( $module, &$params, &$attribs )
{
	if ($module->content)
	{
		...
		if($module->position == 'toolbar') {
			echo '<div class="' . $params->get('moduleclass_sfx') . ' display-dropdown jsn-modulecontainer' . (isset($attribs['columnClass']) ? ' ' . $attribs['columnClass'] : '') . '"><div class="jsn-modulecontainer_inner">';
		} else {
		...
		if (($module->position == 'mainmenu' || $module->position == 'toolbar'))
		{
			if ($module->showtitle) {
				echo '<h3 class="jsn-moduletitle"><span>' . $moduleTitleOuput . '</span></h3>';
			}
			else {
				$moduleTitleOuput = '<span class="jsn-moduleicon no-text"></span>';
